...
Ili9341Display::Ili9341Display(int mosi, int sclk, int cs, int dc, int rst,
                               int bl)
...
void Ili9341Display::init() {
  ...
  if (!backbuffer_) {
    ESP_LOGE(TAG, "Failed to allocate %d-byte backbuffer",
             (int)(backbuffer_len_ * sizeof(uint16_t)));
  } else {
    ESP_LOGI(TAG, "Backbuffer OK: %d bytes",
             (int)(backbuffer_len_ * sizeof(uint16_t)));
  }
  backlightOn();
}
...
void Ili9341Display::setupSPI() {
  ...
  esp_lcd_panel_io_spi_config_t io_config = {.cs_gpio_num = _cs,
                                             .dc_gpio_num = _dc,
                                             .spi_mode = 0,
                                             .pclk_hz =
                                                 30 * 1000 * 1000, // 30 MHz
...
void Ili9341Display::setupPanel() {
  ...
  ESP_ERROR_CHECK(esp_lcd_panel_mirror(
      _panel, true, false)); // parameters are mirror_x and mirror_y
  ...
  if (_panel) {
    ...
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Failed to draw bitmap: %s", esp_err_to_name(err));
        ...
        ESP_LOGI(TAG, "Draw complete."); // temp disable log for beter
    performance
    } */
  } else {
...
bool Ili9341Display::drawPixel(int x, int y, uint16_t color) {
 ...
 *
 */
void Ili9341Display::drawCircle(int x0, int y0, int r, uint16_t color,
                                bool fill) {
  // your code
}
